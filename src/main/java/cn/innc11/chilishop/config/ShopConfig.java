...
import cn.innc11.chilishop.pluginEvent.PlayerRemoveShopEvent;
...
public class ShopConfig extends BaseConfig
{
	...
	public void Reload()
	{
		...
		for(String key : config.getKeys(false))
		{
			...
			shopData.item = Item.fromString(namespaceId);
			shopData.item.setDamage(metadata);
	...
	protected void Save()
	{
		...
		shopDataMapping.forEach((shopKey, shopData)->
		{
			...
			String pureItemName = ChiliShop.ins.itemNameTranslationConfig !=null? ChiliShop.ins.itemNameTranslationConfig.getPureItemName(item):"";
			...
			config.set(shopKey+".item.namespaceId", namespaceId);
