...
public class HologramItemListener implements Listener, ReloadableListener
{
	...
	public HologramItemListener(ChiliShop main)
	{
		ins = main;
		reload();
	...
	{
		for (ShopConfig shopConfig : ChiliShop.ins.shopsConfig.getAllShops())
		{
	...
	{
		if(!force && ChiliShop.ins.pluginConfig.hologramItemEffect<=0) return 0L;
	...
	{
		if(!force && ChiliShop.ins.pluginConfig.hologramItemEffect<=0) return;
