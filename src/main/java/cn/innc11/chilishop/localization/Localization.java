package cn.innc11.chilishop.localization;

import cn.innc11.chilishop.ChiliShop;
import cn.innc11.chilishop.config.BaseConfig;
...
public class Localization extends BaseConfig
{
	...
	public void Reload()
	{
		...
		for(LangNodes key : LangNodes.values())
		{
			...
			if(v==null)
			{
				...
				lang.put(key, field);
				supplement = true;
				ChiliShop.ins.getLogger().info(TextFormat.colorize("&cSet default language text for "+ getCompleteFileName() +"("+key.name()+")"));
			}
		...
		ChiliShop.logger.info(TextFormat.colorize("Loaded &6"+ct+"&r languages from &b"+ getCompleteFileName()));
	}
	...
	public static class SignText
	{
		...
		{
			ItemNameTranslationConfig inc = ChiliShop.ins.itemNameTranslationConfig;
